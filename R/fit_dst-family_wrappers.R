...
#' @param method Estimation method to use. Available built-in options depend on
#'   the distribution family and are enforced via `rlang::arg_match()`. A
#'   composite estimator object from [cqe()] or [cee()] (or the string
#'   shortcuts `"cqe"` / `"cee"`) may also be supplied, and is passed through to
#'   [fit_dst()].
...
fit_dst_bern <- function(x, method = c("mle", "lmom"), ...) {
  if (is.null(as_estimator(method))) {
